...
namespace MIIPL.Common
{
    public class Utils
    {
        ...
        public static string GetTCPHeaderMessage( string strMessage)
        {
            string strFormat = "2H";
            string TCPIPHeaderFormat = string.Empty;
            #region Junk
            try
            {
                short intNum1 = 0;
                ...
                string strBin2 = "";
                TCPIPHeaderFormat = "";
                short intMsgLen = (short)Strings.Len((string)strMessage);
                short intNumber = intMsgLen;
                string strZeros = "00000000";
                
                if (intNumber > 255)
                {
                    goto Label_0105;
                }
                string VB_t_string_L0 = strFormat;
                if (VB_t_string_L0 == "2H")
                {
                    char VB_t_char_S1 = '\0';
                    return (VB_t_char_S1.ToString() + Strings.Chr(intMsgLen).ToString() + strMessage);
                }
                
                return TCPIPHeaderFormat;
            Label_00D3:
                intRemain = (short)(intNumber % 2);
                intNumber = (short)Math.Round(Conversion.Fix((double)(((double)intNumber) / 2.0)));
                strBin = strBin + Conversions.ToString((int)intRemain);
            Label_0105:
                if (intNumber > 0)
                {
                    goto Label_00D3;
                }
                strBin = Strings.StrReverse(strBin);
                strBin1 = Strings.Mid(strBin, 1, Strings.Len(strBin) - 8);
                strBin1 = Strings.Mid(strZeros, 1, 8 - Strings.Len(strBin1)) + strBin1;
                strBin2 = Strings.Right(strBin, 8);
                strBin1 = Strings.StrReverse(strBin1);
                strBin2 = Strings.StrReverse(strBin2);
                short intLoop = 0;
                do
                {
                    intNum1 = (short)Math.Round((double)(intNum1 + (Math.Pow(2.0, (double)intLoop) * Conversion.Val(Strings.Mid(strBin1, intLoop + 1, 1)))));
                    intNum2 = (short)Math.Round((double)(intNum2 + (Math.Pow(2.0, (double)intLoop) * Conversion.Val(Strings.Mid(strBin2, intLoop + 1, 1)))));
                    intLoop = (short)(intLoop + 1);
                    VB_CG_t_i2_S0 = 7;
                }
                while (intLoop <= VB_CG_t_i2_S0);
                switch (strFormat)
                {
                    case "2H":
                        return (Conversions.ToString(Strings.Chr(intNum1)) + Conversions.ToString(Strings.Chr(intNum2)) + strMessage);

                    case "3H":
                        return TCPIPHeaderFormat;

                    case "STX":
                        TCPIPHeaderFormat = "\x0002" + strMessage + "\x0003";
                        break;
                }
            }
            catch (Exception ex)
